...
SELECT name FROM countries WHERE region = 'Southern Europe'AND population = (SELECT MIN(population) FROM countries WHERE region = 'Southern Europe');

-- Clue #2: Now that we're here, we have insight that Carmen was seen attending language classes in this country's officially recognized language. Check our databases and find out what language is spoken in this country, so we can call in a translator to work with you.

-- Write SQL query here

SELECT language FROM countrylanguages INNER JOIN countries ON countrylanguages.countrycode = countries.code WHERE countries.name = 'Holy See (Vatican City State)';
...
SELECT name, countrylanguages.language as language FROM countries INNER JOIN countrylanguages ON countries.code = countrylanguages.countrycode WHERE countrylanguages.language = 'Italian' AND countrylanguages.percentage = 100.0;
...
SELECT cities.name AS city, countries.name FROM cities JOIN countries ON cities.countrycode = countries.code WHERE cities.name LIKE 'Serra%' AND countries.region = 'South America';
...
SELECT cities.name AS capital FROM cities JOIN countries ON countries.code = cities.countrycode WHERE countries.name = 'Brazil' AND cities.id = countries.capital;

-- Clue #7: She knows we're on to her – her taxi dropped her off at the international airport, 
-- and she beat us to the boarding gates. We have one chance to catch her, we just have to know where she's heading and beat her to the landing dock. 
